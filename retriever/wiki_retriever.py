...
topics_and_terms = [
    ("Environment", [
        "Global warming", "Climate change", "Greenhouse gases", "Sea level rise", 
        "Endangered species", "Biodiversity loss", "Wildlife conservation", 
        "Deforestation rates", "Habitat destruction", "Renewable energy", 
        "Plastic pollution", "Air quality"
    ]),

    ("Politics", [
        "Elections", "Public policy analysis", "International relations", 
        "Political parties", "Geopolitics", "Diplomacy", "Legislative processes", 
        "Political campaigns", "Political ideologies", "Lobbying", 
        "Constitutional law", "Human rights"
    ]),

    ("Education", [
        "Literacy rates", "Online education trends", "Student loan data", 
        "Educational technology", "STEM education", "Higher education", 
        "Education policy", "Teacher training", "Early childhood education", 
        "Special education", "Standardized testing", "Curriculum development"
    ]),
    ...
    ("Technology", [
        ...
        "Internet of Things", "Blockchain", "Cryptocurrency", "3D printing", 
        "Quantum computing", "Augmented reality", "5G networks", "Cybersecurity"
    ]),

    ("Economy", [
        "Stock market performance", "Job market trends", "Cryptocurrency", 
        "Economic indicators", "Inflation rates", "GDP growth", "Trade balance", 
        "Interest rates", "Economic forecasts", "Gig economy", "Income inequality", 
        "Sustainable economy"
    ]),

    ("Entertainment", [
        "Music industry", "Streaming platforms", "Box office performance", 
        "TV shows", "Game shows", "Social media influencers", "Streaming wars", 
        "Content creation", "Virtual reality entertainment", "Gaming industry", 
        "Podcasting", "Live event industry"
    ]),
    ...
    if not run_specific_topic:
        for i in range(0, len(topics_terms_batch) - 1, batch_sz):
            ...
            for topic, terms in batch:
                wiki_ret = WikiRetriever(topic, terms, max_workers=20, total_docs=5000)
...
batch_2_results, visited_pages_dict_list2, running_time = main_runner(topics_terms_batch_2)
...
rem_docs_b2 = validate_docs_results(batch_2_results, visited_pages_dict_list2)

if not rem_docs_b2:
    print("No pending docs to retrieve, its all fine for Batch 2")
else:
    print('Fetched this much docs remaining', len(rem_docs_b2))
          

# %%
# Backup Terms for the topic that failed to fetch min of 520 docs
topic_terms_backup =("Travel", [
    "Travel technology", "Hospitality industry", "Travel photography",
    "Travel blogging", "Backpacking", "Road trips",
    "Travel visas", "Travel accommodations", "Travel apps","Travel health"
    ])

# "Travel budgeting", "Travel seasons", "Travel packages",
    # "Travel agents", "Travel reviews", "Travel influencers",
    # "Travel experiences", "Travel destinations", "Travel itineraries",
    # "Travel transportation", "Travel cuisine", "Travel souvenirs"
...
def retrieve_rem_docs(batch_results, visited_pages_dict_list, topic_terms_backup):
    ...
    if docs_retrieve_rem:                                   
        for i in docs_retrieve_rem:
            ...
            remaining_docs_results, visited_pages_dict_list_rem, total_time = main_runner(topics_terms_batch_2, run_specific_topic=True, 
...
rem_doc_results = retrieve_rem_docs(batch_2_results, visited_pages_dict_list2, topic_terms_backup)

for i in batch_2_results:
    print(i['topic'], len(i['documents']))

batch_2_cp = batch_2_results.copy()
...
batch_2_all_results = merge_all_results(batch_2_results, rem_doc_results)

# Extend all_docs with the merged results
all_docs.extend(batch_2_all_results)
...
# Check if each of the topics are having atleast 500 docs
is_min_500 = all(len(i['documents']) > 500 for i in all_docs)

# Validate if all 500 documents under each topic is unique
...
print("Is each topic having 500 documents :", is_min_500)
...
print('All docs temp processing tiem:', e_time - s_time)
...
final_results = batch_results_to_json(all_docs_final, 'wiki_retrieve_docs_1.json')
len(final_results)

# batch_1_dict = batch_results_to_json(batch_1_results)
# batch_2_dict =  batch_results_to_json(batch_2_results)

# final_results = {**batch_1_dict, **batch_2_dict}